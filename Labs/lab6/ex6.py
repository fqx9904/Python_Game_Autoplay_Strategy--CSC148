...
def list_internal(t: Tree) -> list:
    ...
    if not t.children:
        return []
    return gather_lists([[t.value], gather_lists([list_internal(tree)
...
def count_internal(t: Tree) -> int:
    ...
    if not t.children:
        return 0
    return sum([1, sum([count_internal(tree) for tree in t.children])])
...
def count_leaves(t: Tree) -> int:
    ...
    if not t.children:
        return 1
    return sum([0, sum([count_leaves(tree) for tree in t.children])])
...
def sum_internal(t: Tree) -> int:
    ...
    if not t.children:
        return 0
    return sum([t.value, sum([sum_internal(tree) for tree in t.children])])
...
def contains_test_passer(t: Tree, test: Callable[[Any], bool]) -> bool:
    ...
    if not t.children:
        return test(t.value)
    return any([test(t.value), any([contains_test_passer(tree, test)
                                    for tree in t.children])])
...
def list_if(t: Tree, p: Callable[[Any], bool]) -> list:
    ...
    return gather_lists([p(t.value), gather_lists([list_if(tree, p)
                                                   for tree in t.children])])
