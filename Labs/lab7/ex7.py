...
def list_longest_path(node: Union[BTNode, None]) -> list:
    ...
    long = left if len(left) > len(right) else right
    return [node.data] + long
...
def list_between(node: Union[BTNode, None], start: object, end: object) -> list:
    ...
    if not node:
        return []
    return ([node.data] if start <= node.data <= end else []) + \
           (list_between(node.left, start, end) if node.data > start else []) \
        + (list_between(node.right, start, end) if node.data < end else [])
